...
async def read_tasks(project_id: int = None, skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
...
async def update_task(task_id: int, task_update: TaskUpdate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Task).options(selectinload(Task.assignee)).filter(Task.id == task_id))
